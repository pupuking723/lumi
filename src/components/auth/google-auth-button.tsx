...
export function GoogleAuthButton({
  ...
  if (isAuthenticated && session.user) {
    return (
      <div
        ref={menuRef}
        className={cn("relative", compact ? "inline-flex" : "flex w-full", className)}
      >
        <button
          type="button"
        ...
        >
          ...
          <span className="min-w-0 truncate">
            {compact ? userName ?? "Account" : userName ?? userEmail ?? "Account"}
          </span>
          <ChevronDown
            size={14}
            aria-hidden
            className={cn(
              "shrink-0 text-[#7a7289] transition-transform",
              menuOpen && "rotate-180",
            )}
          />
        </button>
        ...
        {menuOpen && (
          <div
            role="menu"
            className={cn(
              "absolute right-0 top-[calc(100%+0.5rem)] z-50 w-64 overflow-hidden rounded-[8px] border border-white/80 bg-[#fffdf8]/96 p-2 text-[#302d43] shadow-[0_18px_48px_rgba(57,49,73,0.18)] backdrop-blur-xl",
              !compact && "left-0 right-auto w-full min-w-64",
            )}
          >
            <div className="flex items-center gap-3 border-b border-[#e8e0dd] px-2 py-2.5">
              ...
              <div className="min-w-0">
                ...
                {userEmail && (
                  <p className="truncate text-xs font-semibold text-[#746d80]">
                    {userEmail}
                  </p>
                )}
              </div>
